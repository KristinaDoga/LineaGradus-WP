<?php
/**
* WC_Gateway_RBSPaymentSber class
*/
if ( ! defined( 'ABSPATH' ) ) {
exit;
}
/**
* RBSPaymentSber Gateway.
* @class    WC_Gateway_RBSPaymentSber
*/
class WC_Gateway_RBSPaymentSber extends WC_Payment_Gateway {
/**
...
$this->method_description = __( 'Allows sber payments.', 'woocommerce-gateway-sber' );
...
$this->allowCallbacks = defined('RBSPAYMENT_SBER_ENABLE_CALLBACK') ? RBSPAYMENT_SBER_ENABLE_CALLBACK : false;
...
add_action( 'woocommerce_scheduled_subscription_payment_sber', array( $this, 'process_subscription_payment' ), 10, 2 );
add_action( 'woocommerce_receipt_' . $this->id, array($this, 'receipt_page'));
add_action( 'woocommerce_api_sber', array($this, 'webhook_result'));
...
if (defined('RBSPAYMENT_SBER_API_VERSION') && RBSPAYMENT_SBER_API_VERSION == 2) {
$token_field = array(
...
if (defined('RBSPAYMENT_SBER_ENABLE_BACK_URL_SETTINGS') && RBSPAYMENT_SBER_ENABLE_BACK_URL_SETTINGS === true) {
...
if (defined('RBSPAYMENT_SBER_ENABLE_CART_OPTIONS') && RBSPAYMENT_SBER_ENABLE_CART_OPTIONS == true) {
...
if (isset($_POST['woocommerce_sber_test_mode'])) {
$action_adr = $this->test_url;
$gate_url = str_replace("payment/rest", "mportal-uat/mvc/public/merchant/update", $action_adr);
if (defined('RBSPAYMENT_SBER_TEST_URL_ALTERNATIVE_DOMAIN')) {
$pattern = '/^https:\/\/[^\/]+/';
$gate_url = preg_replace($pattern, rtrim(RBSPAYMENT_SBER_TEST_URL_ALTERNATIVE_DOMAIN, '/'), $gate_url);
}
} else {
...
$callback_addresses_string = get_option('siteurl') . '?wc-api=sber' . '&action=callback';
if ($this->allowCallbacks !== false) {
...
$result .= "\n\n";
$path = dirname(__FILE__) . '/../logs/wc_sber_' . date('Y-m') . '.log';
...
if (defined('RBSPAYMENT_SBER_ENABLE_BACK_URL_SETTINGS')
&& RBSPAYMENT_SBER_ENABLE_BACK_URL_SETTINGS === true
...
'amount' => $amount,
'returnUrl' => get_option('siteurl') . '?wc-api=sber' . '&action=result&order_id=' . $order_id,
'jsonParams' => json_encode($jsonParams_array),
);
if (defined('RBSPAYMENT_SBER_ENABLE_CART_OPTIONS')
&& RBSPAYMENT_SBER_ENABLE_CART_OPTIONS == true
&& $this->send_order == 'yes') {
...
'measure' => defined('RBSPAYMENT_SBER_MEASUREMENT_NAME') ? RBSPAYMENT_SBER_MEASUREMENT_NAME : 'pcs'
);
} else {
$item['quantity'] = array(
'value' => $value['quantity'],
'measure' => defined('RBSPAYMENT_SBER_MEASUREMENT_CODE') ? RBSPAYMENT_SBER_MEASUREMENT_CODE : '0'
);
}
...
$message = __( 'Order payment failed. To make a successful payment using RBSPaymentSber Payments, please review the gateway settings.', 'woocommerce-gateway-sber' );
